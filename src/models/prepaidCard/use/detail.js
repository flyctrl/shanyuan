...
class Detail extends Component {
  ...
  async _loadUseData(currentPage) {
    ...
    const dataUse = await api.prepaidCard.use.detail.use({ batch, pageSize: 10, currentPage, numberFrom, numberTo }) || {}
    console.log(dataUse)
  ...
  async _loadSendData() {
    ...
    const dataSend = await api.prepaidCard.use.detail.send({ batch, useId }) || {}
    ...
    return (<div><Row className={styles['data-delivery-layout']}>
      ...
      <Col span={12}>
        <div className={styles.left}>
          <p><span>卡片面额</span><span>{dataSend.amount}元</span></p>
      ...
      <div className={styles.btns}>
        ...
        <Button className={styles.btn} type='primary'><a
          href={`${baseUrl}${api.prepaidCard.export}${batch}&numberFrom=${numberFrom}&numberTo=${numberTo}&exportId=${useId}`}
