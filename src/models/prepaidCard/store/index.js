...
class Store extends Component {
  ...
  async _loadList(currentPage) {
    const tableList = await api.prepaidCard.store.list({
      pagesize: '10',
      status: '-1',
      page: currentPage || '1'
    }) || {}
  ...
  handleDel = (e, batch) => {
    ...
    Modal.confirm({
      ...
      onOk() {
        (async (batch) => {
          const data = await api.prepaidCard.store.del({ batch })
  ...
  handleMake = (e, batch) => {
    ...
    Modal.confirm({
      ...
      onOk() {
        (async (batch) => {
          const data = await api.prepaidCard.store.detial1.make({ batch })
          data && _this._loadList()
        })(batch)
      }
    })
      ...
      case 0: // 未制卡
        return (
          <Menu>
            ...
            <Menu.Item>
              <Link to={{
                pathname: urls.PREPAIDCARD_STORE_DETIAL1,
                search: `?batch=${record.batch}`
              }}>详情</Link>
            </Menu.Item>
            <Menu.Item>
      ...
      case 4: // 已全部投放
        return (
          <Menu>
            <Menu.Item>
              <Link to={{
                pathname: urls.PREPAIDCARD_STORE_DETIAL2,
                search: `?batch=${record.batch}`,
              }}>详情</Link>
            </Menu.Item>
          </Menu>
  ...
  render() {
    ...
    return (
      <div>
        <Button
          type='primary'
          style={{ marginBottom: '10px' }}><Link to={urls.PREPAIDCARD_STORE_ADD}>新增</Link></Button>
        <Table
          columns={columns}
          rowKey='id'
