...
class Detail extends Component {
  ...
  async _loadUseData(currentPage) {
    ...
    const dataUse = await api.expenseCard.use.detail.use({ batch, pageSize: 10, currentPage, numberFrom, numberTo }) || {}
    this.setState({ dataUse })
  }

  async _loadSendData() {
    const { batch, useId } = this.state
    const dataSend = await api.expenseCard.use.detail.send({ batch, useId }) || {}
    ...
    return (<div><Row className={styles['data-delivery-layout']}>
      ...
      <div className={styles.btns}>
        ...
        <Button className={styles.btn} type='primary'><a
          href={`${baseUrl}${api.expenseCard.export}${batch}&numberFrom=${numberFrom}&numberTo=${numberTo}&exportId=${useId}`}
