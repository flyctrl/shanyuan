...
class Edit extends Component {
  ...
  handleSubmit = (e) => {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        const data = api.expenseCard.store.edit.save({ ...values, batch })
        if (data) {
          history.push(urls.EXPENSECARD_STORE)
        }
      }
    })
  }
    ...
    return (<div>
      <Form layout='inline' className={styles.form} onSubmit={this.handleSubmit}>
        ...
        <FormItem className={styles.btns}>
          ...
          <Button htmlType='submit' size='default'><Link to={urls.EXPENSECARD_STORE}>取消</Link></Button>
