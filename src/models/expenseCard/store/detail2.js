...
class Detail2 extends Component {
  ...
  async _loadDataStore() {
    ...
    const dataStore = await api.expenseCard.store.detial({ batch }) || {}
    this.setState({ dataStore })
  }

  async _loadDataDelivery() {
    const { batch } = this.state
    const dataDelivery = await api.expenseCard.store.detial2.history({ batch }) || {}
    this.setState({ dataDelivery })
  }

  async _loadDataAddress(value) {
    const dataAddress = await api.expenseCard.store.detial2.address(value) || {}
    let options = []
  ...
  handleSubmit = (e) => {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        ...
        (async (values) => {
          const data = await api.expenseCard.store.detial2.send(values)
  ...
  render() {
    ...
    return (<div>
      <Modal
        ...
        title={<p style={{ textAlign: 'center' }}>消费卡投放</p>}
        onOk={this.handleOk}
      ...
      <Row className={styles['data-store-layout']}>
        ...
        <div className={styles.btns}>
          ...
          <Button className={styles.btn} type='primary'><a
            href={`${baseUrl}${api.expenseCard.export}${batch}`} download>批量导出</a></Button>
