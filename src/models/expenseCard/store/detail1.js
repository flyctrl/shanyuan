...
class Detail1 extends Component {
  ...
  handleMake = (e) => {
    ...
    Modal.confirm({
      ...
      onOk() {
        (async (batch) => {
          await api.expenseCard.store.detial1.make({ batch })
          _this._loadData()
        })(batch)
      }
    })
    ...
    Modal.confirm({
      ...
      onOk() {
        (async (batch) => {
          ...
          if (data) {
            history.push(urls.EXPENSECARD_STORE_DETIAL2 + `?batch=${batch}`)
          }
        })(batch)
      }
    })
        ...
        <div className={styles.btns}>
          ...
          {dataStore.status === 1 ? (<div style={{ display: 'inline-block' }}>
            <Button type='primary' className={styles.btn}>
              <a href={`${baseUrl}${api.expenseCard.exportQr}${batch}`} download>批量导出</a></Button>
