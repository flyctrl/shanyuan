...
class Store extends Component {
  ...
  async _loadList(currentPage) {
    const tableList = await api.expenseCard.store.list({
      pagesize: '10',
      status: '-1',
      page: currentPage || '1'
    }) || {}
  ...
  handleDel = (e, batch) => {
    ...
    Modal.confirm({
      ...
      onOk() {
        (async (batch) => {
          const data = await api.expenseCard.store.del({ batch })
  ...
  handleMake = (e, batch) => {
    ...
    Modal.confirm({
      ...
      onOk() {
        (async (batch) => {
          const data = await api.expenseCard.store.detial1.make({ batch })
          data && _this._loadList()
        })(batch)
      }
    })
  ...
  handleInStore = (e, batch) => {
    ...
    Modal.confirm({
      ...
      onOk() {
        (async (batch) => {
          const data = await api.expenseCard.store.detial1.inStore({ batch })
          data && _this._loadList()
        })(batch)
      }
    })
      ...
      case 0: // 未制卡
        return (
          <Menu>
            ...
            <Menu.Item>
              <Link to={{
                pathname: urls.EXPENSECARD_STORE_EDIT,
                search: `?batch=${record.batch}`,
              }}>修改</Link>
            </Menu.Item>
            <Menu.Item>
              <Link to={{
                pathname: urls.EXPENSECARD_STORE_DETIAL1,
                search: `?batch=${record.batch}`
              }}>详情</Link>
            </Menu.Item>
            <Menu.Item>
      ...
      case 4: // 已全部投放
        return (
          <Menu>
            <Menu.Item>
              <Link to={{
                pathname: urls.EXPENSECARD_STORE_DETIAL2,
                search: `?batch=${record.batch}`,
              }}>详情</Link>
            </Menu.Item>
          </Menu>
  ...
  render() {
    ...
    return (
      <div>
        <Button
          type='primary'
          style={{ marginBottom: '10px' }}><Link to={urls.EXPENSECARD_STORE_ADD}>新增</Link></Button>
        <Table
          columns={columns}
          rowKey='id'
