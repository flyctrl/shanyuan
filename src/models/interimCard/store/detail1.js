...
class Detail1 extends Component {
  ...
  handleMake = (e) => {
    ...
    Modal.confirm({
      ...
      onOk() {
        (async (batch) => {
          await api.interimCard.store.detial1.make({ batch })
          _this._loadData()
        })(batch)
      }
    })
    ...
    Modal.confirm({
      ...
      onOk() {
        (async (batch) => {
          ...
          if (data) {
            history.push(urls.INTERIMCARD_STORE_DETIAL2 + `?batch=${batch}`)
          }
        })(batch)
      }
    })
        ...
        <div className={styles.btns}>
          ...
          {dataStore.status === 1 ? (<div style={{ display: 'inline-block' }}>
            <Button type='primary' className={styles.btn}>
              <a href={`${baseUrl}${api.interimCard.exportQr}${batch}`} download>批量导出</a></Button>
